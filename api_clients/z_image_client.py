...
class ZImageClient:
    def __init__(self, api_key: str = None):
        self.api_key = api_key or os.environ.get("Z_IMAGE_API_KEY", "")
        self.base_url = "https://api.together.xyz/v1/images"

    def generate_image(self, prompt: str, negative_prompt: str = "", width: int = 1024, height: int = 1024, style: str = "Auto", num_images: int = 1):
        """
        Generates images using Z-Image Turbo API or returns simulated high quality frame if API key is not present.
        """
        if not self.api_key:
            logger.info("ZImageClient: Z_IMAGE_API_KEY non specificata. Generazione placeholder/mock Z-Image Turbo per WOX Cinema Studio.")
        ...
        try:
            ...
            logger.warning(f"ZImageClient error: {e}. Fallback to local rendering.")
    ...
    def _generate_fallback_image(self, prompt: str, width: int, height: int, style: str):
        ...
        draw.text((50, 50), "WOX CINEMA STUDIO - Z-IMAGE TURBO", fill=(212, 255, 50))
